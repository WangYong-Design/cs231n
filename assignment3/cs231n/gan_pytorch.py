...
def sample_noise(batch_size, dim, seed=None):
    ...
    output = torch.rand(batch_size,dim)
...
def discriminator_loss(logits_real, logits_fake):
    ...
    # Compute labels corresponding to real or fake
    true_labels  = torch.ones((N,1)).type(dtype)
    false_labels = torch.zeros((N,1)).type(dtype)

    loss_true  = bce_loss(logits_real,true_labels)
    loss_false = bce_loss(logits_fake,false_labels)
